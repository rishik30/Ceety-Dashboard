...
function renderDatabase() {
	...
	const filtered = S.challans.filter(
		(c) =>
	...
	tbody.innerHTML = filtered
		.map((c) => {
			...
			// Build per-piece price summary across all items in this challan
			const ppcSummary = (c.items || [])
				.map((item) => {
					const ppc = calcPerPiecePriceFromItem(item);
					const desc = (item.desc || 'Item').split(' ').slice(0, 2).join(' '); // first 2 words
					return ppc !== null
						? `<div style="white-space:nowrap;">
               <span style="color:var(--text3);font-size:10px;">${desc}:</span>
               <span style="font-weight:500;">${fmtPaise(ppc)}</span>
             </div>`
						: null;
				})
				.filter(Boolean)
    ...
    <tr>
      ...
      <td style="font-family:var(--mono);font-size:12px;line-height:1.8;">
          ${ppcSummary || '—'}
        </td>
      <td style="white-space:nowrap;">
        <button class="btn btn-secondary btn-sm" onclick="printSavedChallan('${c.no}')">Print</button>
        <button class="btn btn-danger btn-sm" onclick="deleteChallan('${c.no}')">Del</button>
      </td>
    </tr>
  `;
		})
...
async function deleteChallan(no) {
	...
	if (S.gasUrl) {
		try {
			await api('deleteChallan', { no });
			toast(`Challan ${no} deleted from Sheet.`);
		} catch (e) {
			toast('Local delete done. Sheet: ' + e.message, 'error');
		}
	}
}
